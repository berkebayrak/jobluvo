...
A call that times out or loses its connection is a call of unknown cost: no usage came back and the provider may have finished and billed it. It is reported as such, no cost row is written, and the message travels into the row's error text so the report can count them; recording the provider's request id against each call waits for the attempt ownership work (item 9).
