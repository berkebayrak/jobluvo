...
**The cost per application supports the pricing if scored per applied stays at or below about ten. Nothing else in the function moves the answer: tailoring and extraction together are under USD 0.0007 at any ratio.**
...
## The function
    ...
    cost per application = scored per applied x 0.000311 + 0.000538 + 0.0021 / applications per user
...
| Tailoring | application, as a change set | 0.000538 per packet, p50 0.000570; 0.00078 with the retries the cited value rule adds | same 100 jobs, edits only, validated | D-013 |
...
- The validator is the product's guarantee, not a cost device (D-013). A value must appear in a fact the line cites; measured over 520 edits before it shipped, 5 rejected, all one invention, 0 legitimate edits failing.
