 ...
 *
 * The sample (src/server/match/sample.ts) is drawn round robin over cells
 * of family, seniority and description length with a fixed seed, so it is
 * repeatable, every cell that exists is represented, and the tailoring
 * sample is the same 100 jobs. It over weights long descriptions against
...
async function main() {
  ...
  const sample = await stratifiedSample(db, userId, facts, n);
  const { chosen } = sample;
  console.log(`candidates ${sample.candidates} in ${sample.cells.length} cells; sample ${chosen.length}`);
