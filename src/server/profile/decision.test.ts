...
describe("the decision body", () => {
  it("reads a replace as a replace, never as an empty decision", () => {
    ...
    // The unbound form is refused: a replacement must say which waiting facts the page showed.
    expect(decisionBody.safeParse({ replaceWith: id }).success).toBe(false);
    expect(decisionBody.safeParse({ replaceWith: id, seen: [id] }).success).toBe(false);
